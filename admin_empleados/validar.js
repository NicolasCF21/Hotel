...
txtSueldo.addEventListener('blur', function(){
    campoSueldo(txtSueldo.value)
}, false)

function emailExiste(email){
    let url = "http://localhost/hotel/config/empleadoAjax.php"
...
function campoApellido(apellido){
  ...
  function campoSueldo(sueldo){
    let url = "http://localhost/hotel/config/empleadoAjax.php"
    let form = new FormData()
    form.append("action", "campoSueldo")
    ...
    .then(data => {
      if(data.ok){
        ...
        document.getElementById('validaSuel').innerHTML = '¡Solo se permiten números!'
      }else{
        document.getElementById('validaSuel').innerHTML = ''
      }
    })
  }
